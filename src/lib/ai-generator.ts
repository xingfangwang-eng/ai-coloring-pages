 ...
 * 当前：133442 —— 砸烂发光南瓜黑光晕缓存（+98765，原 123567 + 98765 = 222332 → 取模后 133442） */
const SEED_CACHE_BUST_OFFSET = 133_442;
...
export function getSanitizedPromptSubject(input: string): string {
  ...
  if (/^[a-z]+-[a-z]+(-[a-z]+)*$/i.test(input)) {
    ...
    const FESTIVE_NEUTRAL_MAP: Record<string, string> = {
      "halloween-pumpkin": "pumpkin with simple cute cartoon face outline", // 纯果蔬卡通简笔（彻底剥离 lantern 发光灯具属性）
 ...
 */
const MAGIC_PROMPT =
  "coloring book page of a {{SUBJECT}}, single 2d contour line drawing, empty uncolored shape, plain stark white paper background";
